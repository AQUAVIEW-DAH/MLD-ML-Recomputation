...
- **Combined WOD-XBT + Argo same-day benchmark**: The combined 1,065-row same-day RTOFS prototype had 48 platform groups. LinearRegression led with mean **MAE=8.509m** and mean **R²=0.076**. This is the best current same-day prototype but still should not be used to freeze `model.pkl`.
...
- **Separate WOD+Argo candidate model artifact**: Updated `train_ml.py` to accept dataset/model/output paths via environment variables and trained a separate `LinearRegression` artifact on `training_data_combined_rtofs_2024_2025.csv`. The candidate artifact `model_wod_argo_rtofs_2024_2025_linear.pkl` achieved **MAE=8.939m** and **R²=0.267** on its held-out grouped split (`876/189` train/test rows across `38/10` platform groups). It remains a candidate artifact only and does not replace `model.pkl`.
- **Pipeline model override + sanity check**: Updated `mld_pipeline.py` to honor `ML_MODEL_PATH` instead of always loading `ML_baseline/model.pkl`, and fixed a stale import to use `extract_erddap_profiles`. A local no-network sanity check using `ML_MODEL_PATH=ML_baseline/model_wod_argo_rtofs_2024_2025_linear.pkl` on `rtofs_glo_3dz_f054_6hrly_hvr_US_east.nc` returned `model_mld=16.96m`, `correction_applied=3.45m`, and `best_estimate_mld=20.42m` at `(28.5, -88.2)`, confirming the candidate artifact can be exercised in the inference path without replacing `model.pkl`.
