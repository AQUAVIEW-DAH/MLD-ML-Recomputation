...
- **`ML_baseline/build_wod_xbt_rtofs_2024_2025.py`**: Added a WOD-XBT-only 2024/2025 same-day Global RTOFS builder/audit path.
- **`ML_baseline/build_argo_gdac_rtofs_2024_2025.py`**: Added an Argo-GDAC 2024/2025 same-day Global RTOFS builder/audit path with a top-N date cap for bounded RTOFS downloads.
- **`ML_baseline/build_erddap_glider_rtofs_2024_2025.py`**: Added a tmux-friendly ERDDAP glider 2024/2025 same-day Global RTOFS builder/audit path.
- **Source/coverage reports**: Added `INSITU_SPATIAL_COVERAGE_REPORT.md`, `PROFILE_METHOD_FIT_SUMMARY.md`, `INSITU_MLD_SOURCE_INVENTORY.md`, `WOD_XBT_RTOFS_2024_2025_REPORT.md`, and `ARGO_GDAC_RTOFS_2024_2025_REPORT.md`.
...
- **WOD XBT 2024/2025 same-day RTOFS prototype**: WOD THREDDS fallback recovered `wod_xbt_2025.nc` after public WOD S3 returned 404. The WOD-XBT audit extracted 138 GoM profiles, found 106 valid 10m temperature-threshold MLD labels under 100m, and retained 83 rows with same-day RTOFS features. Final WOD-XBT rows: 54 from 2024 and 29 from 2025 across 6 platforms and 14 half-degree cells.
- **Argo GDAC 2024/2025 same-day RTOFS audit**: Uncapped Argo GDAC profile-index scan found 5,371 GoM profile files across 51 platforms. The parser extracted 10,165 usable 10m/profile-QC records, 9,177 valid MLD<=100m labels, and 8,894 rows with same-day public RTOFS availability. A bounded top-40-date RTOFS feature pass produced 982 Argo training rows across 42 platforms and 172 half-degree cells.
- **Combined same-day RTOFS prototype**: Combined WOD-XBT and Argo top-40-date rows into `training_data_combined_rtofs_2024_2025.csv` with 1,065 rows across 48 platforms and 55 dates.
- **ERDDAP glider 2024/2025 same-day RTOFS audit**: The tmux run extracted 18,485 ERDDAP glider profiles after profile/10m QC. After enforcing valid 10m-reference MLD labels in the 10-100m range, 13,518 labels remained, and the bounded top-40-date feature pass produced 2,715 same-day RTOFS rows across 12 platforms and 14 half-degree cells.
- **Combined all-source same-day RTOFS prototype**: Combined WOD-XBT, Argo GDAC, and ERDDAP glider rows into `training_data_combined_rtofs_2024_2025_all_sources.csv` with 3,780 rows across 60 platforms and 92 dates.
- **Balanced same-day RTOFS variants**: Added `build_balanced_same_day_datasets.py` and generated three source-balanced variants that keep all WOD-XBT + Argo rows while capping ERDDAP glider density to 1, 2, or 3 rows per platform/date/0.25-degree cell.
...
- **WOD-XBT same-day benchmark**: The separate WOD-XBT 2024/2025 same-day dataset had 83 rows across 6 platforms. RandomForest had the best mean MAE (**9.305m**) but mean R² remained negative (**-0.244**), so this remains a clean prototype only.
- **Argo same-day benchmark**: The Argo GDAC top-40-date same-day dataset had 982 rows across 42 platforms. LinearRegression led with mean **MAE=8.508m** and mean **R²=0.043**, the first positive repeated grouped R² from the same-day RTOFS prototypes.
- **Combined WOD-XBT + Argo same-day benchmark**: The combined 1,065-row same-day RTOFS prototype had 48 platform groups. LinearRegression led with mean **MAE=8.509m** and mean **R²=0.076**. This is the best current same-day prototype and is now the default app baseline in `model.pkl`, while still remaining a prototype rather than a final production acceptance artifact.
- **ERDDAP glider same-day benchmark**: The cleaned 2,715-row ERDDAP glider same-day dataset had 12 platform groups and 14 half-degree cells. LinearRegression led with mean **MAE=3.927m** but mean **R²=-0.196**, indicating source/deployment clustering and weak held-out-platform generalization despite the low absolute error.
- **Combined all-source same-day benchmark**: The 3,780-row WOD-XBT + Argo + ERDDAP same-day prototype had 60 platform groups. LinearRegression led with mean **MAE=5.186m** and mean **R²=-0.050**. Adding ERDDAP increases row count substantially but does not beat the Argo+WOD positive-R² prototype under grouped validation.
- **Balanced ERDDAP benchmark sweep**: ERDDAP cap1/cap2/cap3 variants retained 1,158/1,243/1,327 rows. LinearRegression led all three with mean **MAE=7.827m/R²=0.022**, **MAE=7.715m/R²=-0.017**, and **MAE=7.647m/R²=-0.050**, respectively. Cap1 is the only balanced ERDDAP variant with positive mean R², but it still does not beat the WOD-XBT + Argo baseline mean R² of **0.076**.
- **WOD+Argo prototype baseline artifact**: Updated `train_ml.py` to accept dataset/model/output paths via environment variables and trained a separate `LinearRegression` artifact on `training_data_combined_rtofs_2024_2025.csv`. The artifact `model_wod_argo_rtofs_2024_2025_linear.pkl` achieved **MAE=8.939m** and **R²=0.267** on its held-out grouped split (`876/189` train/test rows across `38/10` platform groups). It has now been promoted into `ML_baseline/model.pkl` so the app uses the WOD+Argo prototype baseline by default.
- **Pipeline default-baseline alignment + sanity check**: Updated `mld_pipeline.py` to honor `ML_MODEL_PATH` instead of always loading `ML_baseline/model.pkl`, fixed a stale import to use `extract_erddap_profiles`, and then promoted the WOD+Argo prototype into `ML_baseline/model.pkl` so the app path no longer depends on the override. A local no-network sanity check on `rtofs_glo_3dz_f054_6hrly_hvr_US_east.nc` returned `model_mld=16.96m`, `correction_applied=3.45m`, and `best_estimate_mld=20.42m` at `(28.5, -88.2)`, confirming the default app baseline loads and runs correctly.
- **Historical replay split + frozen sandbox artifact**: Added `build_historical_replay_split.py` and `train_historical_replay.py` to create a strict replay-safe split for the latest dense same-day window. The chosen replay holdout is **2025-07-07 to 2025-08-31**, producing `training_data_train_historical_replay_pre_2025_07_07.csv` (796 rows; 713 Argo + 83 WOD) and `training_data_holdout_historical_replay_2025_jul_aug.csv` (269 Argo rows across 11 dates and 21 platforms). The frozen replay artifact `model_historical_replay_2025_jul_aug.pkl` achieved **MAE=6.431m, RMSE=8.355m, R²=0.186** on the holdout, improving over the raw RTOFS holdout baseline (**MAE=7.112m, RMSE=9.274m, R²=-0.003**).
